...
# per-system particles
class System_PTS1(Game.System):
    def __init__(self, D, B, T, Npar_sys):
        Game.System.__init__(self, D, B, T)
    ...
    def update_particle_weights(self, c, a, obs, t):
        """
    ...
    def calculate_likelihood(self, theta, c, a, obs):
        ...
        lh = 1.0
        for i in range(self.D):
            p = theta[i][int(a[i])].dot(c)  # the parameter p of a Bernoulli distribution
            if obs[i] == 1:
                lh = lh * p
            else:
                lh = lh * (1-p)
...
class System_PTS2(Game.System):
    ...
    def update_particle_weights(self, c, a, obs, t):
        ...
        for i in range(self.D):  # in each domain, we only update the particles' weights for the block used, while the particles' weights for other blocks remain unchanged
            new_w = np.zeros(self.Npar)  # the unnormalized new weight vector
            for k in range(self.Npar):
                lh = self.calculate_likelihood_for_block(self.Particles[i][int(a[i])][k], c, obs[i])
                #print('likelihood =', lh)
                new_w[k] = lh * self.W[i][int(a[i])][k]
            new_W = 1.0/(np.sum(new_w)) * new_w   # normalizing
            #print('new_W =', new_W)
            self.W[i][int(a[i])] = new_W        
